...
class TextureMapper {
    ...
    private var captureInterval: TimeInterval = 0.8
    ...
    func captureFrame(from arFrame: ARFrame) {
        ...
        if capturedFrames.count >= maxFrames {
            // Remove frames that are most similar in viewing angle to neighbors
    ...
    func sampleVertexColors(vertices: [SIMD3<Float>], normals: [SIMD3<Float>]) -> [SIMD4<Float>] {
        ...
        // Pre-compute camera positions and forward vectors
        ...
        for i in 0..<vertices.count {
            ...
            for (fi, frame) in capturedFrames.enumerated() {
                ...
                // Face must be visible from camera (normal facing camera)
                let normalDot = -dot(normal, toVertexNorm)
                guard normalDot > 0.1 else { continue }

                // Vertex must be in front of camera
                let viewDot = dot(toVertexNorm, cam.forward)
                guard viewDot > 0.3 else { continue }

                // Check if vertex projects into frame bounds
                guard let uv = projectPoint(vertex, into: frame) else { continue }

                // Prefer: close to image center, good normal alignment, reasonable distance
    ...
    func buildTextureAtlas(meshData: MeshData, tileSize: Int = 768) -> TextureAtlasResult? {
        ...
        let atlasWidth = gridCols * tileSize
        let atlasHeight = gridRows * tileSize
        ...
        // For each face, find the best camera frame and compute UVs
        var uvCoordinates = [SIMD2<Float>](repeating: SIMD2<Float>(0, 0), count: meshData.vertices.count)
        var vertexFrameAssignment = [Int](repeating: -1, count: meshData.vertices.count)
        ...
        for face in meshData.faces {
            ...
            let faceNormal = normalize(n0 + n1 + n2)
            ...
            // Project vertices and map to atlas coordinates
            for idx in [i0, i1, i2] {
                // Only assign if not already assigned or if this face has a better frame
                ...
                if let localUV = projectPoint(vertex, into: capturedFrames[bestFrame]) {
                    ...
                    vertexFrameAssignment[idx] = bestFrame
                }
            }
        }
    ...
    func projectPoint(_ worldPoint: SIMD3<Float>, into frame: CapturedFrame) -> SIMD2<Float>? {
        ...
        // Point must be in front of camera (ARKit camera looks along -Z in camera space)
    ...
    private func removeRedundantFrame() {
        guard capturedFrames.count >= 2 else { return }
